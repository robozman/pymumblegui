...
class SignalSlotHandler(QtCore.QObject):
    ...
    def recieve_message(self, message_text, username):
        ...
        coursor.setPosition(len(self.mumble_gui.channel_chat_view.toPlainText()))
        self.mumble_gui.channel_chat_view.setTextCursor(coursor)
        self.mumble_gui.channel_chat_view.insertPlainText(message_text)
...
class MumbleGUI:
    ...
    def __init__(self):
        ...
        self.window.setStyleSheet(open('MetroMumble/MetroDark.qss', 'r').read())
        self.window.show()
        self.centralwidget = self.window.centralwidget


        self.tabbed_chat = self.centralwidget.findChild(QtWidgets.QTabWidget, 'tabbed_chat')
        #self.tabbed_chat.setTabsClosable(True)


        self.channel_chat_view = self.centralwidget.findChild(QtWidgets.QTextEdit, "channel_chat")
        ...
        self.centralwidget.findChild(QtWidgets.QScrollArea, 'scrollArea_2').setStyleSheet("background-color:#1f1f1f;")
    ...
    def populate_channel_list(self, mumble_client):
        ...
        self.root_sub_channels = [v for k, v in mumble_client.mumble.channels.items() if k != 0]
        ...
        for channel in self.root_sub_channels:
            ...
            channel_to_add = QtWidgets.QTreeWidgetItem([channel['name'], str(channel['channel_id']), 'channel', str(channel['position'])])
    ...
    def __change_user_tree_icon(self, user, mumble_client, icon):
        user_to_change = self.channel_view.findItems(str(user['session']), QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 1)[0]
        if icon == 'muted':
            user_to_change.setIcon(0, QtGui.QIcon("MetroMumble/muted_self.svg"))
        elif icon == 'normal':
            user_to_change.setIcon(0, QtGui.QIcon("MetroMumble/talking_off.svg"))
        elif icon == 'deaf':
            user_to_change.setIcon(0, QtGui.QIcon("MetroMumble/deafened_self.svg"))
        elif icon == 'talking':
            user_to_change.setIcon(0, QtGui.QIcon("MetroMumble/talking_on.svg"))
    ...
    def __add_user_to_tree(self, user, mumble_client):
        ...
        #print(user)
        user_to_add = QtWidgets.QTreeWidgetItem([user['name'], str(user['session']), 'user'])
        channel_name = mumble_client.mumble.channels[channel_id]['name']
        channel_widget = self.channel_view.findItems(str(channel_id), QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 1)[0]
        ...
        if 'self_deaf' in user:
            if user['self_deaf']:
                user_to_add.setIcon(0, QtGui.QIcon("MetroMumble/deafened_self.svg"))
                #self.__change_user_tree_icon(user, mumble_client, 'deaf')
        elif 'self_mute' in user:
            if user['self_mute']:
                user_to_add.setIcon(0, QtGui.QIcon("MetroMumble/muted_self.svg"))
                #self.__change_user_tree_icon(user, mumble_client, 'muted')
        else:
    ...
    def __delete_user_from_tree(self, user, mumble_client):
        ...
        #user_name = user['name']
        channel_widget = self.channel_view.findItems(str(channel_id), QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 1)[0]
        user_widget = self.channel_view.findItems(str(user['session']), QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 1)[0]
        channel_widget.removeChild(user_widget)
        

    def __setup_connect_dialog(self):
        self.connect_dialog = uic.loadUi('connect_dialog.ui')
        self.connect_dialog.setStyleSheet(open('MetroMumble/MetroDark.qss', 'r').read())
        ...
        self.favorites_tree.setIcon(0, QtGui.QIcon("MetroMumble/emblems/emblem-favorite.svg"))
        ...
        self.connect_dialog.findChild(QtWidgets.QPushButton, 'add_new_button').clicked.connect(self.__add_favorite_gui)
        self.connect_dialog.findChild(QtWidgets.QPushButton, 'quit_button').clicked.connect(lambda: sys.exit(0))
    ...
    def show_connect_dialog(self, mumble_client):
        ...
        self.connect_dialog.connect_button.clicked.connect(functools.partial(self.__make_mumble_client_connect, mumble_client))
    ...
    def __add_favorite_gui(self):
        ...
        self.add_fav.setStyleSheet(open('MetroMumble/MetroDark.qss', 'r').read())
        self.add_fav.buttonBox.buttons()[0].clicked.connect(self.__add_favorite)
        self.add_fav.exec()
    ...
    def __add_favorite(self):
        ...
        self.favorites_file[address] = {'address': address, 'port': port, 'user': username}
...
class MumbleClient:
    ...
    def connect(self, host, port, user):
        ...
        self.mumble.callbacks.set_callback(pymumble.constants.PYMUMBLE_CLBK_SOUNDRECEIVED, self.on_sound_received)
    ...
    def send_message(self, client_gui):
        ...
        message_post = '{}: {}\n'.format(self.user, client_gui.chat_entry.text())
        client_gui.chat_entry.setText('')
        self.sender.message_recieved.emit(message_post, self.user, client_gui.tabbed_chat.currentIndex())
    ...
    def on_message_recieved(self, message):
        ...
        message_post = '{}: {}\n'.format(self.mumble.users[message.actor]['name'], message.message)
        self.sender.message_recieved.emit(message_post, None, None)
    def set_mumble_gui(self, mumble_gui):
        self.mumble_gui = mumble_gui
        self.mumble.callbacks.set_callback(pymumble.constants.PYMUMBLE_CLBK_USERUPDATED, functools.partial(self.mumble_gui.user_modified, self))
        self.mumble.callbacks.set_callback(pymumble.constants.PYMUMBLE_CLBK_USERCREATED, functools.partial(self.mumble_gui.user_created, self))
        self.mumble.callbacks.set_callback(pymumble.constants.PYMUMBLE_CLBK_USERREMOVED, functools.partial(self.mumble_gui.user_deleted, self))
